package com.jay.trading.lib.util.yahoo;
...
import com.jay.trading.lib.csv.CsvReader;
import com.jay.trading.lib.series.DoubleSeries;
import com.jay.trading.lib.util.HistoricalPriceService;
import com.jay.trading.lib.util.Http;
...
public class YahooFinance implements HistoricalPriceService {
  ...
  public static final CsvReader.ParseFunction<Instant> DATE_COLUMN =
      CsvReader.ParseFunction.ofColumn("Date")
          .map(s -> LocalDate.from(DateTimeFormatter.ISO_DATE.parse(s))
              .atStartOfDay(ZoneOffset.UTC.normalized())
              .toInstant());
  public static final CsvReader.ParseFunction<Double> CLOSE_COLUMN = CsvReader.ParseFunction.doubleColumn("Close");
  public static final CsvReader.ParseFunction<Double> HIGH_COLUMN = CsvReader.ParseFunction.doubleColumn("High");
  public static final CsvReader.ParseFunction<Double> LOW_COLUMN = CsvReader.ParseFunction.doubleColumn("Low");
  public static final CsvReader.ParseFunction<Double> OPEN_COLUMN = CsvReader.ParseFunction.doubleColumn("Open");
  public static final CsvReader.ParseFunction<Double> ADJ_COLUMN = CsvReader.ParseFunction.doubleColumn("Adj Close");
  public static final CsvReader.ParseFunction<Double> VOLUME_COLUMN = CsvReader.ParseFunction.doubleColumn("Volume");
